...
export default function StorePage() {
  ...
  useEffect(() => {
    ...
    const setMeta = (attr: string, value: string) => {
      let el = document.querySelector(`meta[${attr}]`) as HTMLMetaElement;
      if (!el) {
        el = document.createElement('meta');
        el.setAttribute(attr.startsWith('property') ? 'property' : 'name', attr.replace(/^(name|property):/, ''));
    ...
    setMeta('property:og:title', `壽司郎 ${store.name} - 即時等候時間 | 壽司郎 HK Live`);
    setMeta('property:og:description', desc);
    setMeta('property:og:url', `${SITE_URL}/store/${store.id}`);
    ...
    <div className="min-h-screen flex flex-col bg-slate-100/70 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans antialiased">
      ...
      <div className="shrink-0 p-4 sm:p-6 bg-[#141414] text-white relative border-b-4 border-[#aa151b]">
        ...
        <div className="absolute top-4 right-4 z-10">
          <Link
            to="/"
            className="p-2 rounded-full bg-neutral-800 hover:bg-neutral-700 text-white transition-all inline-flex items-center gap-1 active:scale-90"
            aria-label="返回主頁"
          >
