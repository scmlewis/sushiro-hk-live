...
describe('getTicketStatusInfo', () => {
  it('returns 非營業中 when store is not OPEN', () => {
...
describe('isStoreServicing', () => {
  ...
  it('returns false for closed store', () => {
   ...
   it('returns false for stopped store (停籌) with no waiting groups', () => {
    const store = {
      ...baseStore,
...
describe('getStoreDisplayStatus', () => {
  ...
  it('returns 非營業中 for non-OPEN store', () => {
...
it('returns 停籌 for OPEN + local OFF + 0 wait + 0 group', () => {
